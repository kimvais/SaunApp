...
module Utils =
    ...
    let humandate (dt: DateTime) =
        let span = DateTime.UtcNow.Subtract(dt)
        describeTimeElapsed
            (Math.Round(span.TotalMinutes), Math.Round(span.TotalHours), Math.Round(span.TotalDays),
             Math.Round(span.TotalMinutes / 43829.0639), Math.Round(span.TotalMinutes / 525948.766))
